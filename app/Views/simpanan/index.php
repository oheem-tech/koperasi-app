...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2>Data Transaksi Simpanan</h2>
    <?php if(has_permission('manage_simpanan')): ?>
    <a href="<?= base_url('simpanan/create') ?>" class="btn btn-primary"><i class="fas fa-plus"></i> Input Transaksi</a>
    <?php endif; ?>
</div>
...
<?php if(has_permission('manage_simpanan')): ?>
<?php
// Build label for the currently selected member
$selectedLabel = '';
if ($selected_anggota) {
...
?>
<form method="get" id="filterFormSimpanan" class="mb-3">
    <div class="d-flex gap-2 align-items-center flex-wrap">
        <div class="member-search-wrap position-relative" style="min-width:280px;">
            <div class="input-group input-group-sm">
                <span class="input-group-text bg-white border-end-0"><i class="fas fa-search text-muted"></i></span>
                <input type="text" id="simpananSearchInput" class="form-control border-start-0 ps-0"
                    placeholder="Cari nama / no. anggota..."
                    value="<?= esc($selectedLabel) ?>"
                    autocomplete="off">
            </div>
            <input type="hidden" name="anggota_id" id="simpananAnggotaId" value="<?= $selected_anggota ?>">
            <div id="simpananSuggestions" class="member-suggestions shadow"></div>
        </div>
        <button type="submit" class="btn btn-sm btn-primary px-3"><i class="fas fa-filter me-1"></i>Filter</button>
        <?php if($selected_anggota): ?>
            <a href="<?= base_url('simpanan') ?>" class="btn btn-sm btn-outline-secondary"><i class="fas fa-times me-1"></i>Reset</a>
        <?php endif; ?>
    </div>
</form>
...
(function() {
    ...
    var inp = document.getElementById('simpananSearchInput');
    var hid = document.getElementById('simpananAnggotaId');
    var box = document.getElementById('simpananSuggestions');
    var activeIdx = -1;
    ...
    function renderSuggestions(q) {
        ...
        filtered.forEach(function(a, i) {
            ...
            div.addEventListener('mousedown', function(e) { e.preventDefault(); selectMember(a); });
            box.appendChild(div);
        });
    ...
    <div class="card-body overflow-auto">
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <?php if($row['jenis_transaksi'] == 'setor'): ?>
                            <span class="badge bg-success"><i class="fas fa-arrow-down"></i> Setor (Masuk)</span>
                        <?php else: ?>
                            <span class="badge bg-danger"><i class="fas fa-arrow-up"></i> Tarik (Keluar)</span>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="text-center text-nowrap">
                        <a href="<?= base_url('simpanan/print/'.$row['id']) ?>" target="_blank"
                           class="btn btn-sm btn-outline-secondary" title="Cetak Kwitansi">
                            <i class="fas fa-print"></i>
                        </a>
                        <?php if(has_permission('manage_simpanan')): ?>
                        <a href="<?= base_url('simpanan/edit/'.$row['id']) ?>" class="btn btn-sm btn-outline-warning" title="Edit Transaksi">
                            <i class="fas fa-edit"></i>
                        </a>
                        <a href="<?= base_url('simpanan/delete/'.$row['id']) ?>" class="btn btn-sm btn-outline-danger" title="Hapus Transaksi" onclick="return confirm('Apakah Anda yakin ingin menghapus transaksi simpanan ini? Pencatatan pada Kas Koperasi juga akan dibatalkan/dihapus secara otomatis.');">
                ...
                <tr>
                    <td colspan="6" class="text-center">Belum ada data transaksi simpanan.</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
